...
"""
    carbon.notifiers.email.email_notification
...
class EmailNotificationPlugin(NotificationPlugin):
        ...
        try:
            ...
            if self.creds_params.get('smtp_starttls', False) and self.creds_params.get('smtp_starttls') == 'True':
                if smtp.has_extn('STARTTLS'):
                    ...
                    CarbonNotifierError('The Start TLS is not available for the server.')
            ...
            if self.creds_params.get('smtp_user', False) and self.creds_params.get('smtp_password', False):
                if smtp.has_extn('AUTH'):
                    ...
                    raise CarbonNotifierError('Authentication is not available for the server.')
    ...
    def _build_msg(self):
        ...
        mail["User-Agent"] = "carbon-email-notifier"
    ...
    def notify(self):
        ...
        # no msg body params assume using carbon default template
        if not self.body and not self.body_tmpl:
            self.logger.info('Using generic carbon email template')
        ...
        if self.creds_params:
            ...
            raise CarbonNotifierError('No credentials for the Email Notifier was found in the carbon.cfg.')
