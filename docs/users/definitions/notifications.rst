...
Carbon's notification section declares what messages are to be sent
...
notification task within carbon. This task is translated into a carbon notification
...
By default, Carbon implicitly triggers on both **on_success** and **on_failure** for all completed
task types. If you would like to set it for either/or, you can explicitly set either parameter to **true**.

If you would like to have carbon trigger notifications before the start of a task rather than after,
you can set **on_start** to **true**. The **on_start** option is mutually exclusive to
**on_success**/**on_failure**.

If you would like to have carbon not trigger notifications automatically and you would like to control
...
to a list of any combination of supported carbon tasks. This does not apply to **on_demand**.
...
.. code-block:: bash
    ...
    carbon run -s scenario.yml -w . -t provision --skip-notify notification_a --skip-notify notification_b
...
.. code-block:: bash
    ...
    carbon run -s scenario.yml -w . -t execute -t report --no-notify
...
configuration in your carbon.cfg file, see `SMTP Configuration
...
.. list-table::
    ...
    *   - cc
        - The list of email addresses that you want to send carbon copies to.
        - List
        - False
    ...
    *   - message_body
        - The text body of the message to include overriding Carbon's default message template.
        - String
        - False

    *   - message_template
        - A relative path to a text email body template in Carbon's workspace that should be used.
          It overrides Carbon's default message template.
        - String
        - False
...
Carbon has a default messaging template that is sent when no **message_body** or **message_template**
parameter is used. Carbon uses some internal data about the tasks performed by the scenario.
Below is the list of data being rendered into the message

 * overall status of the Scenario execution
 * The list of Carbon tasks that passed and/or failed
